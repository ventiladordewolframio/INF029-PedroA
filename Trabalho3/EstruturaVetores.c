...
void addElementoEstruturaAuxiliar(int posicao, int isNULL, int conteudo) {
    int qtdElem = getQuantidadeElementosEstruturaAuxiliar(posicao);

    if (!isNULL) {
        printf("insert %d in %d|", conteudo, posicao);
        if (qtdElem == SEM_ESTRUTURA_AUXILIAR) {
            criarEstruturaAuxiliar(posicao, 1);
        }
        int espaco = inserirNumeroEmEstrutura(posicao, conteudo);
        if (espaco == SEM_ESPACO) {
            modificarTamanhoEstruturaAuxiliar(posicao, 1);
            inserirNumeroEmEstrutura(posicao, conteudo);
        }
    } else {
        printf("insert NULL in %d|", posicao);
        if (qtdElem == SEM_ESTRUTURA_AUXILIAR) {
            criarEstruturaAuxiliar(posicao, 1);
        } else {
            modificarTamanhoEstruturaAuxiliar(posicao, 1);
...
void inicializar() {
    FILE *file = fopen("estruturas.dat", "r+");
    char *save1;
    char *save2;
    if (file) {
        printf("File exists.\n");
        ...
        while ((ch = getc(file)) != EOF) {
            buff[i++] = (char)ch;  // Store the character in the buffer
        }
        // printf(buff);
        // printf("\n\n\n|=>");
        ...
        while (token != NULL) {
            if ((strcmp(token, "END") == 0)) {  // se vazio
                i = i + 1;
                // printf("LINE BREAK|");
                token = strtok_r(NULL, "\n", &save1);
                continue;
            ...
            while (token != NULL) {
                ...
                if ((strcmp(token, "NULL") == 0)) {
                    // printf("NULL|");
                    addElementoEstruturaAuxiliar(i, 1, 0);
                } else {
                    // printf("NUM%d|",atoi(token));
                    addElementoEstruturaAuxiliar(i, 0, atoi(token));
                }
                token = strtok_r(NULL, " ", &save2);
            }
            printf("\n");
        ...
        // int vect[50];
        // getDadosDeTodasEstruturasAuxiliares(vect);
        // for(int i = 0; i< 50; i++){
        //     printf("|%d",vect[i]);
        // }
        fclose(file);
...
void finalizar() {
    printf("File does not exist.\n");
    FILE *file = fopen("estruturas.dat", "w");
    fclose(file);
    file = fopen("estruturas.dat", "a");
    ...
    for (int i = 0; i < 10; i++) {
        ...
        while (ptr_No != NULL) {
            ...
            if (ptr_No->vazio == 1) {
                ...
                first = 0;
                fprintf(file, "%d", ptr_No->conteudo);
            }
            ptr_No = ptr_No->prox;
        }
        if (i == 9) {
